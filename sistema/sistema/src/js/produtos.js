...
modal.btnSalvar.addEventListener('click', (e) =>{
    ...
    const DATA_REGEX = /^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[012])\/(19|20)\d{2}$/;

    if(DATA_REGEX.test(document.querySelector('#data').value) === false) {

        console.log(data)
        console.log(DATA_REGEX.test(data))
        mostarAlerta("Data preenchida é inválida para o cadastro.");
        return;
    }
